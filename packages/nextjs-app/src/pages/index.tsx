...
import {
  ...
  SlotShadowExample,
  FetchExample,
} from '../stencil-web-components';

import dynamic from 'next/dynamic';

const DynamicStencilFetchComponentWithNoSSR = dynamic(
  () => import('../components/FetchComponent'),
  { ssr: false }
);
...
const Index = () => {
  ...
  return (
    <div className="hero">
      ...
      <div className="mt-8">
        <label>
          <input type="text" onBlur={(e) => setRepo(e.target.value)} />
        </label>
        <DynamicStencilFetchComponentWithNoSSR repo={repo} />
      </div>
    </div>
  );
};
